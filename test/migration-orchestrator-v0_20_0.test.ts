/**
 * v0.20.0 Cathedral II Layer 13 — orchestrator contract tests.
 *
 ...
 * assertions live in test/migrations-v0_20_0.test.ts (pinned the v27
 * migration's SQL shape from Layer 1).
 */

import { describe, test, expect } from 'bun:test';

describe('v0.20.0 orchestrator — Cathedral II migration', () => {
  ...
  test('feature pitch names the headline capabilities', async () => {
    const { v0_20_0 } = await import('../src/commands/migrations/v0_20_0.ts');
    const desc = v0_20_0.featurePitch.description ?? '';
  ...
  test('phase functions exported for unit testing', async () => {
    const { __testing } = await import('../src/commands/migrations/v0_20_0.ts');
  ...
  test('dry-run skips all side-effect phases', async () => {
    const { v0_20_0 } = await import('../src/commands/migrations/v0_20_0.ts');
    const result = await v0_20_0.orchestrator({
      yes: true,
      dryRun: true,
  ...
  test('v0.20.0 is the latest registered migration', async () => {
    const { migrations } = await import('../src/commands/migrations/index.ts');
    const last = migrations[migrations.length - 1]!;
    expect(last.version).toBe('0.20.0');
  });
});
