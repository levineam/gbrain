...
describe('v0.29 — buildBrainTools subagent surfacing', () => {
  // buildBrainTools instantiation needs an engine + config but never
  // touches the DB at registry-build time. PGLite suffices and stays fast.
  let _engine: PGLiteEngine;
  const config: GBrainConfig = { engine: 'pglite' } as GBrainConfig;

  // We never connect — buildBrainTools just reads engine.kind for the
  // namespace-wrap branch and doesn't issue any SQL.
  _engine = new PGLiteEngine();

  test('subagent registry includes brain_get_recent_salience', () => {
    const tools = buildBrainTools({ subagentId: 1, engine: _engine, config });
