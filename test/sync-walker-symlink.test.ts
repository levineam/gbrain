...
let tmp: string;
let originalMultimodal: string | undefined;

beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'gbrain-walker-'));
  originalMultimodal = process.env.GBRAIN_EMBEDDING_MULTIMODAL;
  delete process.env.GBRAIN_EMBEDDING_MULTIMODAL;
});

afterEach(() => {
  if (originalMultimodal !== undefined) {
    process.env.GBRAIN_EMBEDDING_MULTIMODAL = originalMultimodal;
  } else {
    delete process.env.GBRAIN_EMBEDDING_MULTIMODAL;
  }
...
describe('collectSyncableFiles symlink + cycle hardening', () => {
  test('1. self-referencing symlink does not loop', () => {
  ...
  test('2. symlink chain through real dirs does not loop', () => {
  ...
  test('3. max-depth bailout terminates pathological deep trees', () => {
  ...
  test('4. strategy filter admits the right files', () => {
  ...
  test('5. dot-prefixed dirs and node_modules still skipped', () => {
  ...
  test('6. multimodal preservation under markdown strategy (codex C5)', () => {
  ...
  test('7. deterministic ordering — two walks return identical arrays (codex C8)', () => {
